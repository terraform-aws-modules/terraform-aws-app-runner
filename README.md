...
| <a name="input_domain_names"></a> [domain\_names](#input\_domain\_names) | List of custom domain endpoints to associate with the App Runner service. Specify a base domain e.g., `example.com` or a subdomain e.g., `subdomain.example.com` | `list(string)` | `[]` | no |
...
| <a name="input_enable_www_subdomain"></a> [enable\_www\_subdomain](#input\_enable\_www\_subdomain) | Whether to add the www. subdomain for each domain name associated with the App Runner service, in addition to the base domain. Defaults to `true` | `bool` | `null` | no |
